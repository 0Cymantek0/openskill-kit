...
osk.command("learn")
  ...
  .option("--record-concept-outcome <conceptId>", "Record local hashed Learn-v2 concept outcome telemetry for a concept id (query/path/command hashes only, never raw values)")
  .option("--concept-outcome <outcome>", "Concept outcome: helpful|ignored|wrong|harmful|superseded")
  .option("--concept-outcome-reason <text>", "Optional outcome reason; Learn v2 stores only hash/classes/placeholders")
  ...
  .option("--learn-v2-agent-eval <path>", "Validated Learn-v2 behavior-agent eval artifact to include in --run-learn-v2-eval proof boundary")
  ...
  .action(async (options) => {
    ...
    if (options.runLearnV2Eval === true) {
      const result = withLearnV2EvalProofBoundary(await runPersistedLearnV2Eval(process.cwd(), {
        ...
        behaviorAgentEvalPath: options.learnV2AgentEval,
...
function renderLearnV2ObservabilityTui(report: LearnV2PipelineObservabilityReport): void {
  ...
  clackNote([
    ...
    `Snippets: ${report.evidence.declassifiedSnippets} declassified (${renderLearnV2CountLine(report.evidence.snippetResidualRiskCounts)})`,
    `Stitching: ${renderLearnV2CountLine(report.evidence.stitchingMethodCounts)}`,
...
function renderLearnV2PersistedEval(result: Awaited<ReturnType<typeof runPersistedLearnV2Eval>>): string {
  ...
  return [
    ...
    `Behavior agent eval: ${result.summary.behaviorDelta.agentStatus ?? "not-run"} (${result.summary.behaviorDelta.agentResultCount ?? 0} result(s))`,
...
function renderRawVaultMaintenance(result: Awaited<ReturnType<typeof runLearnV2RawVaultMaintenance>>): string {
  return [
    ...
    `Usage ratios: hot ${result.budgetSummary.hotUsageRatio}, pinned ${result.budgetSummary.pinnedUsageRatio}, total ${result.budgetSummary.totalUsageRatio}`,
    `Tier counts: ${renderLearnV2CountLine(result.budgetSummary.tierCounts)}`,
    `Next hot expiry: ${result.budgetSummary.nextHotExpiryAt ?? "none"}`,
    `Hot record range: ${result.budgetSummary.oldestHotRecordAt ?? "none"} -> ${result.budgetSummary.newestHotRecordAt ?? "none"}`,
    `Over-budget reasons: ${result.budgetSummary.overBudgetReasons.join(", ") || "none"}`,
