...
describe("osk CLI facade", () => {
  ...
  it("prints persisted Learn v2 eval summary metrics", async () => {
    ...
    expect(textResult.stdout).toContain("Proves: concept retrieval from stored episodes; deterministic activation scoring; memory-admission activation exclusion for one-off/rejected/superseded concepts; conditional memory admission non-overlearning checks; open-world grounding authority and evidence-separation checks; configured behavior-delta golden checks; deterministic counterfactual trace activation checks; local sandbox verifier command execution");
