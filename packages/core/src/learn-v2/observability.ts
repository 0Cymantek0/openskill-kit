...
export async function writeLearnV2PipelineObservabilityReport(
  ...
  const report = LearnV2PipelineObservabilityReportSchema.parse({
    ...
    skillOntology: {
      ...
      dormantNamespaces: input.skillOntology?.counts.dormantNamespaces ?? 0,
...
function renderPipelineObservabilityReport(report: LearnV2PipelineObservabilityReport): string {
  return [
    ...
    `- Skill namespaces: ${report.skillOntology.namespaces} (${report.skillOntology.candidateNamespaces} candidate, ${report.skillOntology.reviewNamespaces} needs review, ${report.skillOntology.dormantNamespaces} dormant)`,
