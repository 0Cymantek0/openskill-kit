...
export async function applyLearnV2ContradictionReviewOutputs(
  ...
  const outputFiles = (await Promise.all(outputPathsInput.map((file) => resolveContradictionOutputInputPath(root, file, rejected)))).filter((file): file is string => Boolean(file));
...
async function resolveContradictionOutputInputPath(root: string, inputPath: string, rejected: LearnV2ContradictionReviewApplyResult["rejected"]): Promise<string | undefined> {
