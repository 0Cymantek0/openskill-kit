...
export async function routeBehavior(input: RouteBehaviorInput): Promise<BehaviorRoutePlan> {
  ...
  const novelty = estimateNovelty(query, paths, graph.nodes.map((node) => [node.category, node.statement, ...node.scope.paths].join(" ")));
  const localCoverage = estimateLocalCoverage(retrieval.items.length, retrieval.trace.consideredCount, novelty.score);
