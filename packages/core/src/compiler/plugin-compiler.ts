...
function renderOpenCodePlugin(): string {
  return [
    ...
    "    const traceContext = buildTraceContext(projectRoot, metadata, pluginStartedAt, bootTraceSeed);",
    "    const base = {",
    ...
    "}",
    "",
    "function buildTraceContext(projectRoot: string, metadata: Record<string, unknown>, pluginStartedAt: string, seed: string): Record<string, string> {",
    ...
    "    opencodeSessionId: safeOpenCodeSessionId(metadata) ?? safeTraceId(process.env.OPENCODE_SESSION_ID, \"opencode_session\") ?? `opencode_session_${hashBare(`${seed}:opencode`)}`,",
    ...
    "  const raw = stringMetadata(metadata, \"input.sessionID\") ?? stringMetadata(metadata, \"output.sessionID\");",
    "  return raw ? `opencode_session_${hashBare(raw)}` : undefined;",
    "}",
    "",
