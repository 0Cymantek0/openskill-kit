...
export function createOpenSkillMcpServer(options: { profile?: OpenSkillMcpProfile } = {}): McpServer {
  ...
  registerTool(
    ...
    {
      title: "OpenSkillKit Learn v2 Scope Request Preparation",
      description: "Write prompt-safe concept scope bundles and scope-inference prompts for OpenCode-configured agents. Does not call a provider.",
      inputSchema: z.object({
        projectRoot: projectRootSchema,
    ...
    async ({ projectRoot, conceptIds }) => {
      const root = resolveProjectRoot(projectRoot);
      return toolResult(await writeLearnV2ScopeInferenceRequests(root, conceptIds), root);
    }
  );
  ...
  registerTool(
    ...
    {
      title: "OpenSkillKit Learn v2 Eval Planner Request Preparation",
      description: "Write prompt-safe concept eval-planning bundles and eval-planner prompts for OpenCode-configured agents. Does not call a provider.",
      inputSchema: z.object({
        projectRoot: projectRootSchema,
        conceptIds: z.array(z.string().min(1)).default([])
      }),
    ...
    async ({ projectRoot, conceptIds }) => {
      const root = resolveProjectRoot(projectRoot);
      return toolResult(await writeLearnV2EvalPlannerRequests(root, conceptIds), root);
    }
  );
  ...
  registerTool(
    ...
    {
      title: "OpenSkillKit Learn v2 Scope Output Apply",
      description: "Validate OpenCode-routed scope-inferencer JSON outputs against stored Learn v2 concepts and merge accepted scope/activation refinements.",
      inputSchema: z.object({
        projectRoot: projectRootSchema,
        outputPaths: z.array(z.string().min(1)).min(1)
      }),
  ...
  registerTool(
    ...
    {
      title: "OpenSkillKit Learn v2 Eval Planner Output Apply",
      description: "Validate OpenCode-routed eval-planner JSON outputs and write review-required proposed eval golden files without mutating concepts.",
      inputSchema: z.object({
        projectRoot: projectRootSchema,
        outputPaths: z.array(z.string().min(1)).min(1)
      }),
