...
- OpenWorld `retrieval-adapters`, `source-plan`, and `execute-source-plan` expose adapter gates, default execution to dry-run, require explicit approval before ingestion writes, ingest only leakage-audited recommended local candidates plus explicit vetted URLs or opt-in autonomous docs/repo candidates, and write adapter-level execution traces with source/index provenance for adapter, retrieval mode, plan/candidate ids, allowWeb state, operator cache use, and network policy.
