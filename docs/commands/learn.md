...
6. For raw local learning, run the Learn v2 project relevance hard gate, persist the relevance calibration artifact, keep non-accepted sources out of episode/concept extraction, reconstruct task episodes from accepted sources only, compress tool/diff/log evidence from transcripts, terminal output, local CI/JUnit reports, IDE diagnostics, issues, reviews, and docs, extract deterministic behavior atoms, merge concept cards, write a behavior-delta-first review queue, compile preview, and learn-v2 eval report.
   Structured event metadata can provide declassified UI context factors such as CSS classes, component ancestry, AST/component symbols, screenshot labels, and design-token names for conditional hypothesis inference without requiring those factors to appear in the user prompt.
...
- Learn v2 eval includes concept-quality gates for activation surface, broad-scope evidence strength, confidence caps, command-policy extraction, active reliability/durability, counterevidence, risky suppression, memory-admission activation exclusion for one-off/rejected/superseded concepts, conditional-admission non-overlearning checks, open-world grounding authority/precedence/evidence-separation checks, and privacy boundaries. Deterministic extraction and OpenCode model proposal validation both keep explicit one-off preferences as observation/debug evidence only: one-off deterministic preference atoms are rejected before concept merge, model proposals marked `oneOff: true` are rejected before durable atom creation, and pipeline observability reports counts-only rejected atom reason histograms so developers can see why proposed memories were not recorded.
...
- Learn v2 conditional learning derives UI context factors from text, metadata, and path/component names, including path-only hints such as `DarkCardButton.tsx` for dark theme, card containment, and button role. Single-support contrastive hypotheses remain weak observations by default; repeated support or explicit durable language is required before a conditional hypothesis becomes a reviewable concept candidate and behavior atom.
