...
- Learn v2 writes a concept conflict ledger and links it from the review queue so contradictory, superseding, or overlapping concepts stay reviewable instead of hidden in scoring side effects. Conflict entries include declassified diagnostics such as scope overlap, token overlap, kind/polarity checks, confidence delta, supersession pair ids, authority reasons, and protection reasons so reviewers can see why a resolution was suggested.
