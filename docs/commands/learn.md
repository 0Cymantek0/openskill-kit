...
- Learn v2 writes a source-gate review artifact for every raw run. Accepted sources may write normalized analysis evidence and enter extraction. Review-needed sources get only a bounded declassified snippet for human source review. Rejected sources are tombstone-only. Non-accepted sources do not write normalized evidence into analysis frames and cannot mutate the canonical concept store, activation index, review queue, compile preview, or eval inputs.
...
- `--execute-model-requests --apply-model-responses` applies written responses by the executed manifest `modelRole`: concept-extractor responses go through the model proposal applier, scope-inferencer responses go through the scope applier, and contradiction-reviewer responses go through the contradiction applier. The JSON result includes `applyByRole` plus a backwards-compatible primary `apply` object for single-role runs.
...
- Learn v2 structural diff summaries use the TypeScript compiler API for TypeScript/JavaScript declarations/imports and block-aware deterministic scope recovery for Python, Go, and Rust body-only edits, including Python classes, Go receiver methods, and Rust `impl` methods.
...
- Learn v2 patch summaries compare an assistant/agent proposed patch with a later user final or manual-edit patch when both appear in the same reconstructed episode. The comparison records shared/final-only/proposed-only paths, structural classes, symbols, imports, line deltas, confidence, and behavior signals such as user-added-tests, narrowed scope, generated/lockfile removal, API-surface correction, or material rework. Deterministic extraction treats the final/manual-edit side as higher-authority correction evidence.
